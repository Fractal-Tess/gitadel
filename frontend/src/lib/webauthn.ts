...
function descriptors(
  ...
  if (!Array.isArray(value))
    throw new Error("The passkey credential list is invalid.");
...
export function creationOptions(
  ...
  const authenticatorSelection =
    typeof payload.authenticatorSelection === "object" &&
    payload.authenticatorSelection !== null &&
    !Array.isArray(payload.authenticatorSelection)
...
export async function getCredential(
  ...
  let received: Credential | null;
  try {
    received = await navigator.credentials.get({ publicKey: options });
  } catch (caught) {
    console.error("[passkey] browser credential request failed", {
  ...
  if (!(received instanceof PublicKeyCredential))
