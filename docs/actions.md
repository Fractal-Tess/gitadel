...
The standalone `compose.yaml` does not start a runner. Add the optional
`compose.actions.yaml` overlay when you want Gitadel Actions, the pinned
Forgejo Runner, and the isolated Docker-in-Docker daemon:
...
`GITADEL_PUBLIC_URL` is required by the overlay. Set it to the URL that
browser clients use and that the runner can reach. Include the public HTTP
port when it is not the standard port for its scheme. The runner uses this
URL for registration and API requests. Workflow containers run through the
dedicated Docker daemon, not the host Docker socket.

The overlay keeps the existing runner configuration and registration flow. A
one-time registration token generated by Gitadel is consumed from
`runner-data`; no token copying or repository-by-repository setup is required.

Persistent volumes:

- `gitadel-data`: Gitadel's database, repositories, LFS objects, SSH host key,
  and other configured local storage.
- `runner-data`: Forgejo Runner registration and state, including the
  one-time registration token while it is waiting to be consumed.
- `runner-docker`: isolated workflow-container storage.

Check the services and runner:

```bash
docker compose -f compose.yaml -f compose.actions.yaml ps
docker compose -f compose.yaml -f compose.actions.yaml logs runner
```

The runner is ready when its log reports `declared successfully`. Gitadel then
shows it as **Online** under **Administration → Runners**. Stop and restart
the overlay with the same two `-f` options so the services keep using their
named volumes:

```bash
docker compose -f compose.yaml -f compose.actions.yaml stop
docker compose -f compose.yaml -f compose.actions.yaml start
```

Do not use `down -v` on an instance that contains data. It deletes the
Gitadel, runner, and workflow-container volumes.
